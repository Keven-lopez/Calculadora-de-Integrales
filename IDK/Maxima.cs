...
namespace MaximaSharp
{
    public static class Maxima
    {
        ...
        public static LambdaExpression ToExpression(string types, string parameters, string code)
        {
            try
            {
                string result = Regex.Replace(code, @"sqrt\(([^)]+)\)", "Math.Sqrt($1)");
               ...
               @"   Imports System
                    ...
                    Public Class Program 
                        ...
                        End Class
                ", types, parameters, result.Replace("ln", "Math.Log").Replace("log", "Math.Log").Replace("sin", "Math.Sin").Replace("cos", "Math.Cos").
                    Replace("e", "Math.E").Replace("%e", "Math.E").Replace("e^", "Math.Exp(").Replace("inf", "double.PositiveInfinity").Replace("-inf", "double.NegativeInfinity").
                    Replace("asin", "Math.Asin").Replace("sqrt", "Math.Sqrt").Replace("abs","Math.Abs").Replace("atan","Math.Atan").Replace("pi","Math.PI").Replace("%pi","Math.PI")))
        ...
        public static LambdaExpression Integrate(this LambdaExpression expr, double from, double to, string wrt = "x")
        {
            return EvalToExpression(expr, "float(integrate({0}, {1}, {2}, {3}))", expr.Body, wrt, from, to);
        }

        public static LambdaExpression IntegrateValor(this LambdaExpression expr, double from, double to, string wrt = "x")
        {
            return EvalToExpression(expr, "(1 / ({3} - {2})) * integrate({0}, {1}, {2}, {3})", expr.Body, wrt, from, to);
        }
